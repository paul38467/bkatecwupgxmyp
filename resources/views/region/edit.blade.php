...
@section('content')
    ...
    @component('components.edit_delete_view')
        ...
        <label for="regionRegionName">地區 ID: {{ $region->id }}</label>
        <input type="text" id="regionRegionName" name="region_name" value="{{ old('region_name', $region->region_name) }}" class="form-control ml-2" placeholder="輸入地區名稱" aria-label="輸入地區名稱">
    @endcomponent
@endsection
